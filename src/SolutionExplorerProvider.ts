...
export class SolutionExplorerProvider extends vscode.Disposable implements vscode.TreeDataProvider<sln.TreeItem> {
    ...
    private _activeNode: sln.TreeItem;
    ...
    constructor(private readonly context: vscode.ExtensionContext, public workspaceRoots: string[], public readonly eventAggregator: IEventAggegator) {
        super(() => this.dispose());
        this._logger = new Logger(this.eventAggregator);
        this._templateEngines = [];
        context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(() => this.onActiveEditorChanged()));
        context.subscriptions.push(vscode.window.onDidChangeVisibleTextEditors(data => this.onVisibleEditorsChanged(data)));
    }
    ...
    public get activeNode(): sln.TreeItem {
    ...
    public register() {
        ...
        if (showMode !== SolutionExplorerConfiguration.SHOW_MODE_NONE) {
            ...
            this.context.subscriptions.push(this.treeView.onDidChangeSelection(this.onSelectionChanged, this));
            this.context.subscriptions.push(this.treeView.onDidCollapseElement(this.onExpansionChanged, this));
            this.context.subscriptions.push(this.treeView.onDidExpandElement(this.onExpansionChanged, this));
        }
    }
    ...
    private onSelectionChanged(event: vscode.TreeViewSelectionChangeEvent<sln.TreeItem>) {
        this._activeNode = event.selection.length > 0 ? event.selection[0] : undefined;
    }

    private onExpansionChanged(event: vscode.TreeViewExpansionEvent<sln.TreeItem>) {
        this._activeNode = event.element;
    }
